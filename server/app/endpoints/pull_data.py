...
from app.models.jira_reader import JiraDataLoader
...
def fetch_data(user_id: str):
    ...
    for user_data_source in user_data_sources:
        ...
        # if user_data_source.data_source.type == DataSourceType.jira:
        #     upload_jira_data(decrypt_data(base64.b64decode(user_data_source.credentials)), collection_name="integrations")
...
def upload_jira_data(token, collection_name):
    """_summary_
    ...
    documents = JiraDataLoader(api_token=token).load_data()

    print(f"jira document = {documents}")
